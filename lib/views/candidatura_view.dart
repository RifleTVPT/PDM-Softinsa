...
class CandidaturaView extends StatefulWidget {
  final int? idBadge;
  const CandidaturaView({super.key, this.idBadge});
...
class _CandidaturaViewState extends State<CandidaturaView> {
  ...
  void _atualizarAreasPorSL(String sl) {
    ...
    if (sl == "Todas as Service Lines") {
      _todasAreas = ["Todas as Áreas", ..._listaBadgesCatalogo.map((e) => e['area'].toString()).toSet()];
    } else {
      _todasAreas = ["Todas as Áreas", ..._listaBadgesCatalogo.where((e) => e['sl'] == sl).map((e) => e['area'].toString()).toSet()];
    }
    _atualizarNiveis();
  }
  ...
  Future<void> _guardarRascunhoFicheiros() async {
    ...
    List<String> paraGuardar = _ficheirosAnexados.map((f) => jsonEncode({
      'name': f.name,
      'path': f.path,
      'size': f.size,
    })).toList();
  ...
  void _verificarRequisitosConcluidos() {
    ...
    setState(() {
      ...
      for (var ficheiro in _ficheirosAnexados) {
        String? reqMapeado = FileValidator.extrairRequisitoDoNome(ficheiro.name);
        if (reqMapeado != null) {
          // Procurar o requisito com este ID e mapear
          for (var req in _badgeSelecionado!['requisitos']) {
            if (req['id'].toString().toUpperCase() == reqMapeado.toUpperCase()) {
              req['concluido'] = true;
  ...
  Future<void> _selecionarBadge(int idBadge) async {
    ...
    final dados = await BDLocalAjudante().obterBadgeDetalhe(idBadge, 1); // 1 = fallback utilizador
    if (!mounted) return;
    ...
    if (dados != null) {
      ...
      if (dados['requisitos'] != null) {
        for (var req in dados['requisitos']) {
          requisitosMutaveis.add({
            'id': req['titulo']?.toString() ?? req['id'].toString(),
            'desc': req['desc']?.toString() ?? '',
            'concluido': false, // Novo pedido, nenhum requisito está concluído ainda
      ...
      setState(() {
        ...
        _passoAtual = 2; // Avança para o passo 2
      });
  ...
  void _avancar() async {
    ...
    if (_passoAtual == 3) {
      ...
      Map<String, dynamic> pedidoPayload = {
        ...
        'ID_BADGE': _badgeSelecionado!['id'] ?? 1, // Fallback caso o mock não tenha ID
      ...
      List<dynamic> requisitosDoBadge = _badgeSelecionado!['requisitos'] ?? [];

      for (PlatformFile file in _ficheirosAnexados) {
        if (file.path == null) continue;
        
        String? reqMapeadoStr = FileValidator.extrairRequisitoDoNome(file.name);
        int? idRequisitoEncontrado;

        if (reqMapeadoStr != null) {
          for (var req in requisitosDoBadge) {
            String titulo = req['titulo']?.toString() ?? req['id']?.toString() ?? '';
            if (titulo.toUpperCase() == reqMapeadoStr.toUpperCase()) {
              idRequisitoEncontrado = req['id'];
              break;
            }
          }
        }
        ...
        final Map<String, dynamic> evidenciaObj = {
          ...
          'REQUISITO_MAPEADO': idRequisitoEncontrado, // Pode ser null se não mapeado, o backend trata
      ...
      if (temNet) {
        ...
        bool sucesso = await ApiServico().enviarPedido(pedidoPayload);
        if (sucesso) {
          // Marca como sincronizado localmente
          final db = await bd.database;
          await db.update('PEDIDO', {'IS_SINCRONIZADO': 1, 'ESTADO_PEDIDO': 'Em Análise'}, where: 'ID_PEDIDO = ?', whereArgs: [novoIdPedido]);
        }
      }
      ...
      await Future.delayed(const Duration(milliseconds: 600)); // Simula tempo de rede no mock
    }
  ...
  List<Map<String, dynamic>> _obterBadgesFiltrados() {
    return _listaBadgesCatalogo.where((badge) {
      ...
      bool areaMatch = _areaEscolhida == "Todas as Áreas" || badge["area"] == _areaEscolhida;
                            ...
                            children: [
                              Container(
                                ...
                                child: const Icon(Icons.workspace_premium,
                                    color: Color(0xFF34659D), size: 30), // Placeholder icon
                              ),
                                  ...
                                  children: [
                                    Text(_ultimoPedido!['NOME_BADGE'],
                                        style: const TextStyle(
                                            fontWeight: FontWeight.bold,
                                            fontSize: 16)),
                                    Text("${_ultimoPedido!['SERVICE_LINE']} / ${_ultimoPedido!['NOME_AREA']} - Nível ${_ultimoPedido!['NOME_NIVEL']}",
                                        style: const TextStyle(
                                            color: Colors.blue,
                                            fontSize: 13,
                          ...
                          Row(
                            ...
                            children: [
                              Container(
                                ...
                                decoration: BoxDecoration(
                                    color: _obterCorStatus(_ultimoPedido!['ESTADO_PEDIDO']).withOpacity(0.1),
                                    borderRadius: BorderRadius.circular(20)),
                                child: Text(_ultimoPedido!['ESTADO_PEDIDO'],
                                    style: TextStyle(
                                        color: _obterCorStatus(_ultimoPedido!['ESTADO_PEDIDO']),
                                        fontWeight: FontWeight.bold,
                                        fontSize: 11)),
                              ),
                              TextButton(
                                  onPressed: () => context.push('/pedido_status', extra: {'idPedido': _ultimoPedido!['ID_PEDIDO']}),
                      ...
                      Container(
                        ...
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<String>(
                            ...
                            isExpanded: true,
                            items: ["Todas as Service Lines", ..._listaBadgesCatalogo.map((e) => e['sl'].toString()).toSet()]
                                .map((s) =>
                              ...
                              showDialog(
                                ...
                                builder: (BuildContext context) {
                                  return AlertDialog(
                                    ...
                                    content: const Text('Já possui este badge. Deseja ver os detalhes deste badge ou voltar à página de candidaturas?'),
                                    actions: [
                                      TextButton(
                                        onPressed: () {
                                      ...
                                      TextButton(
                                        onPressed: () {
                                          Navigator.of(context).pop();
                                          context.push('/badge_detalhe', extra: {'idBadge': badge['id']});
                                        },
                                        child: const Text('Ver Detalhes'),
                                      ),
                                    ],
                              ...
                              children: [
                                Row(
                                  children: [
                                    Container(
                                      ...
                                      child: const Icon(Icons.workspace_premium, color: Color(0xFF34659D), size: 30), // Placeholder icone
                                    ),
                                    const SizedBox(width: 15),
                                    Expanded(
                                      child: Column(
                                ...
                                Row(
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        const Icon(Icons.task_alt, color: Colors.grey, size: 16),
                                    ...
                                    Container(
                                      ...
                                      decoration: BoxDecoration(
                                        color: badge['obtido'] == true 
                                            ...
                                            : const Color(0xFF0980E9).withOpacity(0.1),
                                        borderRadius: BorderRadius.circular(20),
                                      ),
                                      child: Text(
                                          badge['obtido'] == true ? "Badge já obtido" : "Candidatar",
                                          style: TextStyle(
                                              fontWeight: FontWeight.bold,
                                              fontSize: 13,
                                              color: badge['obtido'] == true 
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            "${_badgeSelecionado!['sl'] ?? ''} | ${_badgeSelecionado!['area'] ?? ''} | Nível ${_badgeSelecionado!['nivel'] ?? ''}",
            textAlign: TextAlign.center,
            style: const TextStyle(
                fontSize: 14,
                fontWeight: FontWeight.bold,
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                backgroundColor: botaoAtivo ? const Color(0xFF0980E9) : Colors.grey,
      ...
      child: Column(
        ...
        children: [
            ...
            child: Row(
              ...
              children: [
                ...
                Expanded(
                  child: RichText(
                    text: const TextSpan(
                      style: TextStyle(fontSize: 13, color: Color(0xFF34659D), height: 1.4),
                      children: [
                        TextSpan(text: "Regra de Associação Automática: ", style: TextStyle(fontWeight: FontWeight.bold)),
                        TextSpan(text: "Para que o sistema valide corretamente as suas evidências, certifique-se que o nome do ficheiro inclui o título do requisito que está a submeter (ex: se o requisito for \"A1\", o ficheiro deve conter \"A1\" no nome). Pode submeter vários ficheiros para o mesmo requisito."),
                      ]
                    )
                  )
                )
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(15),
                border: Border.all(
                    color: const Color(0xFFE9EEF2), width: 1.5),
                boxShadow: [
                ...
                children: [
                  ...
                  Text("Arraste e Largue aqui os seus ficheiros",
                      ...
                      style: TextStyle(
                          ...
                          fontSize: 16)),
                  SizedBox(height: 5),
                  Text("Suporta múltiplos ficheiros em simultâneo (PDF, PNG, JPG...)",
                      textAlign: TextAlign.center,
          ...
          if (_ficheirosAnexados.isEmpty)
             const Text("Ainda não foram adicionados ficheiros.", style: TextStyle(color: Colors.grey, fontSize: 13))
          else
            ...ficheirosNaoMapeados.map((f) => Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: const Text("Não Mapeado", style: TextStyle(color: Colors.red, fontSize: 10, fontWeight: FontWeight.bold)),
                        )
                      ],
                    )
                  ),
          ...
          SingleChildScrollView(
            ...
            child: Row(
              ...
              children: _badgeSelecionado!['requisitos'].map<Widget>((r) {
                bool ok = r['concluido'];
                List<PlatformFile> anexos = r['ficheiros'] as List<PlatformFile>;
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(
                    color: Colors.white,
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(color: ok ? Colors.green.withOpacity(0.5) : Colors.red.withOpacity(0.3)),
                  ),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Row(
                        children: [
                          Icon(Icons.assignment, color: ok ? Colors.green : Colors.red.withOpacity(0.5), size: 20),
                          const SizedBox(width: 8),
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text("Requisito ${r['id']}", style: const TextStyle(fontSize: 12, color: Colors.grey, fontWeight: FontWeight.bold)),
                                Text(ok ? "Mapeado" : "Pendente...", style: TextStyle(color: ok ? Colors.green : Colors.red, fontWeight: FontWeight.bold, fontSize: 14)),
                              ],
                            ),
                          )
                        ],
                      ...
                      if (anexos.isNotEmpty) ...[
                        ...
                        ...anexos.map((anexo) => Padding(
                          ...
                          child: Row(
                            children: [
                              const Icon(Icons.file_present, size: 14, color: Colors.blueGrey),
                              const SizedBox(width: 5),
                              Expanded(child: Text(anexo.name, style: const TextStyle(fontSize: 11, color: Colors.blueGrey), maxLines: 1, overflow: TextOverflow.ellipsis)),
                              InkWell(
                                onTap: () => _removerFicheiroAnexado(anexo),
          ...
          SizedBox(
            ...
            height: 55,
            child: ElevatedButton(
              onPressed: (todosRequisitosConcluidos && _termosAceites) ? _avancar : null,
              style: ElevatedButton.styleFrom(
                backgroundColor: (todosRequisitosConcluidos && _termosAceites) ? Colors.green : Colors.grey.shade300,
                elevation: 0,
                shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(10)),
              ),
